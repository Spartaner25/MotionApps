...
public abstract class WindowOrientationListener {
    ...
    private WindowOrientationListener(Context context, int rate) {
        ...
        Slog.v("Sensors", "getting SCREEN ORIENTATION");
        mSensor = mSensorManager.getDefaultSensor(Sensor.TYPE_SCREEN_ORIENTATION);
        if (mSensor == null) {
            mSensor = mSensorManager.getDefaultSensor(USE_GRAVITY_SENSOR
                    ? Sensor.TYPE_GRAVITY : Sensor.TYPE_ACCELEROMETER);
        } else {
            Slog.v("Sensors", "SCREEN ORIENTATION enabled");
        }
    ...
    static final class SensorEventListenerImpl implements SensorEventListener {
        ...
        public void onSensorChanged(SensorEvent event) {
            if (event.sensor.getType() == Sensor.TYPE_SCREEN_ORIENTATION) {
                int intTemp = (int) event.values[0];
                for (int i = 0; i < 2; i++) {
                    updatePredictedRotation(event.timestamp + 5 * i, intTemp);
                    mOrientationListener.onProposedRotationChanged(intTemp);
                    mProposedRotation = intTemp;
                }
                return;
            }
